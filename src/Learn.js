...
const Learn = ({ isLoggedIn, username: usernameProp = '', userRole = '', handleLogout }) => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        setIsLoading(true);
        const response = await fetch('https://untrumpeted-sallie-shallowly.ngrok-free.dev/compiler/course-details/', {
                            ...
                            <div className="w3-example-actions">
                              <button
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    const response = await fetch('https://untrumpeted-sallie-shallowly.ngrok-free.dev/compiler/practice-run/', {
                                      method: 'POST',
                                      headers: {
